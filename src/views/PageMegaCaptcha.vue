<template>
  <div class="captcha">
    <BaseTitle class="captcha__title" title="Докажи, что ты достоин" red :icon="IconType.HEAD" />
...
export default defineComponent({
  name: "PageMegaCaptcha",
  components: { BaseDescription, BaseButton, BaseTitle },
  setup() {
    const MAX_STEP = 0;
    const solutions = [[2, 3, 5]];

    const subtitles = ["Выбери все изображения, где есть ошибки в коде"];
    ...
    const dynamicTiles = computed(() => {
      ...
      for (let i = 1; i <= 9; i++) {
        arr.push({
          id: i,
          src: `/img/megaCaptcha/${step.value + 1}/${i}.jpg`,
        });
      }
      return arr;
    });
    ...
    const onCheckButtonClick = () => {
      ...
      if (!puzzleSolved.value) {
        ...
        router.push({ name: "complete", query: { quest: "megaCaptcha" } });
      }
    };
